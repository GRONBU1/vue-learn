...
import com.vuelearn.pro.util.JsonResult;
import com.vuelearn.pro.util.R;
...
public class VlearnprodController {
    ...
    @ResponseBody
    public R testResult1(ServletResponse res) {
        HttpServletResponse httpServletResponse = (HttpServletResponse) res;
        httpServletResponse.setHeader("Access-Control-Allow-Origin", "*");
        Map<String, Object> data = new HashMap<>();
        List<Vlearnprod> list=vlearnprodService.list();
        return R.ok("state", "message", data).put("taskItems", list);
    }
